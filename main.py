...
def determine_tasks_to_run(cnfg: dict, data_saver: Callable[[Channel], None], fetcher: PostsFetcher):
    ...
    if cnfg["function"] == "get_last_n_posts":
        if "num" in cnfg["params"]:
            ...
            tasks = [fetcher.get_last_n_posts(channel, num, data_saver) for channel in channels]
        else:
        ...
        if "from_date" in cnfg["params"] and "to_date" in cnfg["params"]:
            ...
            tasks = [fetcher.get_posts_by_date_range(channel, from_date, to_date, data_saver) for channel in channels]
        else:
        ...
        if "date" in cnfg["params"]:
            ...
            tasks = [fetcher.get_posts_by_date(channel, date, data_saver) for channel in channels]
        else:
            logger.error(f"No 'date' key in cnfg['params']")
    elif cnfg["function"] == "get_post_by_id":
        if "pid" in cnfg["params"]:
            pid = cnfg["params"]["pid"]
            tasks = [fetcher.get_posts_by_date(channel, pid, data_saver) for channel in channels]
        else:
